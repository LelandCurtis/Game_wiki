...
RSpec.describe Role, type: :model do
  ...
  describe 'class methods' do
    before :each do
      ...
      @weapon_3 = Weapon.create!(name: 'Sword', ranged_attack: true, fire_rate: 0.25, damage: 15, role_id: @role_2.id)
      @weapon_4 = Weapon.create!(name: 'Popsicle', ranged_attack: true, fire_rate: 0.25, damage: 15, role_id: @role_2.id)
    end

    describe '::by_weapon_count' do
      it 'returns all roles sorted by the number of weapons each has' do
        expect(Role.by_weapon_count).to eq([@role_2, @role_1])
      end
    end
  end
